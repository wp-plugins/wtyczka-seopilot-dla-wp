...
class SeoPilot
{
	...
	function SeoPilot()
	{
		...
		self::$client = new SeoPilotClient( array(
		...
		add_action('admin_menu',  array (&$this, 'SeoPilot_Admin_Menu') );

		add_action('wp_head', array (__CLASS__, 'addStyleToHead'));

		if (function_exists ('register_widget') )
		{
			add_action('widgets_init', create_function('', 'return register_widget("SeoPilot_Widget");'));
		}

		if (function_exists ('add_shortcode') )
		{
			add_shortcode('seopilot_build_links', array(__CLASS__, 'getLinks'));
			add_shortcode('seopilot_build_links_is', array(__CLASS__, 'getLinksIs'));
		}
	}
	...
	static function getLinks($atts, $content = null)
	{
		...
		$orientation = null;
		if(isset($atts['orientation']) && in_array($atts['orientation'], array('v', 'h', 's'))) {
	...
	function SeoPilot_Admin_Menu() {
		add_menu_page( 'SeoPilot', 'SeoPilot', 'manage_options', 'seopilot-admin-menu', array(__CLASS__, 'SeoPilot_Admin_Options') );
	}
